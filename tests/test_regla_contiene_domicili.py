...
import pandas as pd
import pytest
from desagregacion_dsg_upc.rules.contiene_domicili import ReglaDescripcionDomicili

@pytest.fixture
def sample_df() -> pd.DataFrame:
    """DataFrame de ejemplo para los tests de reglas que contienen 'DOMICILI'."""
    data = {
        "DESCRIPCION_CUP": [
            "VISITA DOMICILIARIA",  # Cumple: Contiene DOMICILI
            "CONSULTA MEDICA GENERAL",  # No Cumple
            "ATENCION DOMICILIARIA POR ENFERMERIA",  # Cumple
            "SERVICIO DE URGENCIAS",  # No Cumple
            "OXIGENO DOMICILIARIO",  # Cumple
        ],
...
def test_identificar_registros_correctos(settings_mock, sample_df):
    """Verifica que la regla identifique solo las descripciones que contienen DOMICILI."""
    rule = ReglaDescripcionDomicili()
...
def test_desagregacion_completa_domicili(settings_mock, sample_df):
    """
    Test de punta a punta que ejecuta la desagregación de registros DOMICILI y valida el resultado.
